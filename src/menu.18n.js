export const menu18n = {
    hu: {
        food: {
            titles: {
                rec: 'Állófogadás',
                smol: 'Sütemények',
                menu: 'Vacsora',
                cake: 'Torta',
                midnight: 'Éjféli menü',
            },
            rec: [
                'Aszalt szilvával töltött pulykamell',
                'Fasírozott',
                'Sült csirkecomb',
                'Sonkatekercs tormakrémmel töltve',
                'Debreceni karaj',
                'Baconba göngyölt csirkemáj',
                'Gyümölcstál'
            ],
            smol: [
                'Citromtart',
                'Áfonya-fehércsoki tart',
                'Pisztácia-málna choux',
                'Linzer',
                'Sajtos pogácsa',
                'Fetás-aszalt paradicsomos pogácsa'
            ],
            menu: [
                'Tyúkhúsleves, főtt hús, zöldségek',
                '',
                'Sültes tál',
                '- Szatmári tölött flekken -',
                '- Rántott sertéskaraj - ',
                '- Sonkával, sajttal töltött rántott pulykamell -',
                '- Sült csirkecomb -',
                '- Rántott karfiol -',
                '- Rántott sajt - ',
                '- Francia köret -',
            ],
            cake: [
                '- mák-citrom-fehércsoki -',
                '- sós karamell-mogyoró -',
                '- étcsoki trüffel -',
            ],
            midnight: [
                'Flekken sütés',
                '- Csirkemell -',
                '- Fűszeres tarja -',
                '- Görög saláta -',
            ],
        }
    },
    en: {
        food: {
            titles: {
                rec: 'Reception',
                smol: 'Desserts & Pastries',
                menu: 'Diner',
                cake: 'Cake',
                midnight: 'Midnight menu',
            },
            rec: [
                'Turkey Breast Stuffed with Dried Plums',
                'Minced Meat Patty',
                'Roasted Chicken Leg',
                'Ham Roll Filled with Horseradish Cream',
                'Debrecen-Style Pork Loin',
                'Chicken Liver Wrapped in Bacon',
                'Fruit Platter',
            ],
            smol: [
                'Lemon Tart',
                'Blueberry-White Chocolate Tart',
                'Pistachio-Raspberry Choux',
                'Linzer Cookie',
                'Cheese Scone',
                'Feta and Sun-Dried Tomato Scone',
            ],
            menu: [
                'Chicken consommé, vegetables',
                '',
                'Main',
                '- Stuffed meat à la Szatmár :) -',
                '- Breaded prok loin - ',
                '- Breaded turkey breast filled with ham and cheese -',
                '- Roasted chicken leg -',
                '- Breaded cauliflower -',
                '- Breaded cheese - ',
                '- Steamed rice -',
                '- Roasted potatoes -',
                '- Vegetables -',
            ],
            cake: [
                '- Poppy Seed-Lemon-White Chocolate -',
                '- Salted Caramel-Peanut -',
                '- Dark Chocolate Truffle -',
            ],
            midnight: [
                'Grill',
                '- Chicken breast -',
                '- Spicky pork ribs -',
                '- Greek salad -',
            ],
        }
    }
}
